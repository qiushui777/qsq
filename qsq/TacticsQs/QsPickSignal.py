...
def signal_backtest_percent(account=None ,buysignal=None, sellsignal=None):
    ...
    for date_index in df.index:
        ...
        if account.symbol_amount(sell_result['symbol']) > 0 and close_price < account.stop_loss_price[sell_result['symbol']]:
            ...
            logging.info("The asset is " + str(account.balance + account.symbol_amount(sell_result['symbol'])*close_price))

        # 设置下帐户的资产
        account.asset.loc[date_index] = account.balance + account.symbol_amount(sell_result['symbol'])*close_price